...
class yeomanWorkflow.Views.StatEditorView extends Backbone.Fixins.SuperView
  events:
  ...
  renderStatControls: =>
    ...
    unit_stats.each (stat, i) =>
      @$(".stats").append(new window.yeomanWorkflow.Views.StatControlView(unit: @model, model: stat).render().el)
  ...
  show: (unit) =>
    ...
    @model.set("stats", new window.yeomanWorkflow.Models.UnitStatModel(@model.get('stats').toJSON()))
