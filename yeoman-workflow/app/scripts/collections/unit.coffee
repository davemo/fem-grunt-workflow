...
class yeomanWorkflow.Collections.UnitCollection extends Backbone.Collection
  model: yeomanWorkflow.Models.UnitModel
  ...
  deserialize: (encoded) =>
    ...
    _(units).each (unit_data) =>
      if unit_data[4].length is 0
        model_data.push(new window.yeomanWorkflow.Models.UnitModel)
      else
        model_data.push(
          ...
          stats: new window.yeomanWorkflow.Collections.UnitStatCollection(
